...
namespace geg {
  ...
  if(mat.pbrMetallicRoughness.metallicRoughnessTexture.index >= 0){
	  tinygltf::Texture mt = file.textures[mat.pbrMetallicRoughness.metallicRoughnessTexture.index];
  ...
  if(mat.normalTexture.index >= 0) {
  	...
  	pbr_c.normal_map = enqueue_texture(path, false, 4);
        ...
        Entity entt = scene->create_entity();
	entt.add_component<components::PBR>(pbr_c);
        ...
        switch (accessor.componentType) {
          ...
          default:
            GEG_CORE_ASSERT(false, "unsupported index");
        }

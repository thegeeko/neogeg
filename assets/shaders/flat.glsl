...
void main() {
	...
	vec3 F0 = vec3(0.04);
	F0 = mix(F0, oubo.albedo, oubo.metallic);
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		float NDF = distributionGGX(ws_norm, H, oubo.roughness);
		float G = geometry_smith(ws_norm, ws_pos, L, oubo.roughness);
		vec3 F = fresnel_schlick(max(dot(H, ws_pos), 0.0), F0);

		vec3 kS = F;
		vec3 kD = vec3(1.0) - kS;
		kD *= 1.0 - oubo.metallic;
		...
		Lo += (kD * oubo.albedo / PI + specular) * radiance * NdotL;
	}

	vec3 ambient = vec3(0.03) * oubo.albedo * oubo.ao;
